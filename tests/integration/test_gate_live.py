...
EXPECTED_TOOLS = {
    "db_schema", "db_query",
    "files_list_dir", "files_read_file", "files_write_file",
    "web_search", "web_fetch",
    "sandbox_run_python",
    "actions_send_email", "actions_create_calendar_event", "actions_call_api",
}
...
async def test_unauthorised_and_destructive_calls_never_execute(registry: ToolRegistry, settings) -> None:  # type: ignore[no-untyped-def]
    gate = Gate(registry, make_rate_limiter(settings), classifier=StaticClassifier("approve"))
    # research is not allowed the sandbox
    d = await gate.decide_async("research", ToolCall(id="1", name="sandbox_run_python", arguments={"code": "print(1)"}))
    ...
    d = await gate.decide_async(
        "writing",
        ToolCall(id="2", name="actions_send_email", arguments={"to": "lender@example.test", "subject": "s", "body": "b"}),
    )
    ...
    d = await gate.decide_async("analysis", ToolCall(id="4", name="sandbox_run_python", arguments={"code": 42}))
...
async def test_destructive_tool_only_queues_to_outbox(registry: ToolRegistry, settings) -> None:  # type: ignore[no-untyped-def]
    ...
    result = await registry.invoke(
        ToolCall(id="5", name="actions_send_email", arguments={"to": "lender@example.test", "subject": "Complaint CLM-4471", "body": "Draft"})
    )
...
async def test_sandbox_has_no_network_and_is_killed_on_timeout(registry: ToolRegistry) -> None:
    ...
    call = ToolCall(id="6", name="sandbox_run_python", arguments={"code": code_net, "timeout_s": 15})
    assert (await gate.decide_async("analysis", call, context="B: compute")).action == GateAction.ALLOW
    ...
    compute = await registry.invoke(
        ToolCall(id="7", name="sandbox_run_python", arguments={"code": "import pandas as pd; print(pd.Series([150,180,216]).sum())"})
    )
    assert not compute.is_error and "546" in compute.content

    hang = await registry.invoke(
        ToolCall(id="8", name="sandbox_run_python", arguments={"code": "import time; time.sleep(60)", "timeout_s": 3})
    )
    ...
    escape = await registry.invoke(
        ToolCall(id="9", name="sandbox_run_python", arguments={"code": "open('/etc/hostname','w').write('x')"})
    )
    assert not escape.is_error and ("Read-only file system" in escape.content or "Permission denied" in escape.content)
